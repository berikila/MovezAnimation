...
class Animation {
	constructor(selector) {
		_defineProperty(this, "cols", 6);
		_defineProperty(this, "rows", 4);
		_defineProperty(this, "lineWidth", 40);
		_defineProperty(this, "markerName", "marker");
		_defineProperty(this, "spacingHorizontal", 100);
		_defineProperty(this, "spacingVertical", 100);
		_defineProperty(
			this,
			"strokeColor",
			getComputedStyle(document.documentElement).getPropertyValue("--arrows")
		);
		_defineProperty(this, "strokeWidth", 10);
		_defineProperty(this, "svgMargin", 40);
		_defineProperty(this, "svgHeight", 0);
		_defineProperty(this, "svgWidth", 0);
		...
		_defineProperty(this, "mouseStored", Object.assign({}, this.mouse));
	...
	createMarker() {
		...
		marker.setAttribute(
			"viewBox",
			`0 0 ${this.strokeWidth} ${this.strokeWidth}`
		);
		...
		path.setAttribute(
			"d",
			`M 0 0 L ${this.strokeWidth} ${this.strokeWidth / 2} L 0 ${
				this.strokeWidth
			} z`
		);
	...
	setLineRotation() {
		...
		this.lines.forEach((line) => {
			...
			let angle =
				Math.atan2(
					this.mouse.x - line.center.x,
					-(this.mouse.y - line.center.y)
				) *
					(180 / Math.PI) -
				90;
...
class Line {
	constructor(x1, x2, y1, y2, strokeColor, strokeWidth, marker) {
		...
		_defineProperty(this, "strokeColor", "white");
	...
	setElement(x1, x2, y1, y2, strokeColor, strokeWidth, marker) {
		...
		this.element.setAttribute(
			"marker-end",
			marker ? `url(#${marker})` : `url(#${this.marker})`
		);
	}
}
